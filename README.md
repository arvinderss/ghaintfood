...
**Before publishing, fill the `TODO`/placeholder markers in `legal.html`** — `[LEGAL ENTITY NAME]`, the registered address, `[EMAIL]`, and the `[DATE]` "Last updated" value. Note these appear **once per language block**, so fill all three (en/hi/pa). Have the text reviewed by a qualified professional (it is a plain-language starting point, not legal advice).
