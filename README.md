...
`legal.html` holds the **Terms & Conditions** and **Privacy Policy**, linked from both footers. It reuses `index.html`'s header/footer chrome and the same three scripts, so the theme toggle, language switch and footer year work with no extra code; it carries the same CSP.

The legal copy is provided in all three languages (**English / हिंदी / ਪੰਜਾਬੀ**), with **English as the authoritative version** (stated in a precedence note under the title). Rather than routing this long-form text through `i18n.js`, each topic is a language-**neutral** `<section>` carrying the single id (`#security`/`#privacy`/`#terms`), and inside it are three `.legal-lang` blocks (`data-lang="en|hi|pa"`). A CSS rule shows only the block matching `<html lang>` — which `main.js` already sets from the language switch — so **ids stay unique, and the footer/TOC anchors keep working in every language**. English is the static default, so it also shows before JS runs and with JS disabled. To edit a clause, update all three language blocks (and keep English authoritative).
...
**Before publishing, fill the remaining `TODO`/placeholder markers in `legal.html`** — `[LEGAL ENTITY NAME]`, the registered address, and the `[DATE]` "Last updated" value. Note these appear **once per language block**, so fill all three (en/hi/pa). The contact email is already set (`support@ghaintfood.com`). Have the text reviewed by a qualified professional (it is a plain-language starting point, not legal advice).
